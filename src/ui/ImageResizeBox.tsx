...
class ImageResizeBoxControl extends React.Component<any, any> {
    props: {
        boxID: string
        direction: string
        height: number
        onResizeEnd: (w: number, height: number) => void
        config: (el: HTMLElement, width: number, height: number) => void
        width: number
    }
    ...
    render() {
        ...
        return <span role="presentation" className={className} onMouseDown={this._onMouseDown} />
    }
...
class ImageResizeBox extends React.Component<any, any> {
    props: Props

    _id = uuid()

    render() {
        const {onResizeEnd, width, height, src, alt, title} = this.props
        ...
        return (
            <span className={prefixed("image-resize-box")} id={boxID} style={style}>
                {controls}
                <img className={prefixed("image-resize-box-image")} src={src} alt={alt} title={title} />
            </span>
        )
    }
}
